...
class BuyFilter:
    ...
    def name(self) -> str:
        return (
            ...
            f"wpos{self.week_close_to_high_30_min:.2f}_depth{self.base2_depth_max:.2f}"
        ).replace(".", "p")
...
BUY_FILTERS = [
    BuyFilter(rs, amt, wpos, depth)
    for rs in [0.50, 0.60, 0.70, 0.80]
    for amt in [1.00, 1.20, 1.50]
    for wpos in [0.75, 0.80, 0.85]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="base_on_base 主线策略实验室")
...
def evaluate_one_signal(signal, prices: pd.DataFrame, exit_rule: ExitRule) -> dict:
    ...
    base.update(
        {
            ...
            "entry_gap_from_signal_close": entry_price / float(signal.close) - 1,
            "entry_over_pivot": entry_price / pivot - 1,
            "valid_entry": True,
        }
    )
    ...
    for hold_idx, row in enumerate(future.itertuples(index=False), start=0):
        ...
        reason = None
        if close <= entry_price * (1 - exit_rule.hard_stop):
            reason = "hard_stop"
...
def main() -> None:
    ...
    summary_path = lab_dir / f"base_on_base_strategy_grid_{stamp}.csv"
    summary.to_csv(summary_path, index=False)

    best = summary.head(20).copy()
    best_path = lab_dir / f"base_on_base_top20_{stamp}.csv"
    ...
    for _, row in best.head(5).iterrows():
        ...
        detail_path = lab_dir / f"validation_{label[:120]}_{stamp}.csv"
        validation.to_csv(detail_path, index=False)
        detail_paths.append(detail_path)

    year_path = lab_dir / f"base_on_base_top5_by_year_{stamp}.csv"
    reasons_path = lab_dir / f"base_on_base_top5_exit_reasons_{stamp}.csv"
