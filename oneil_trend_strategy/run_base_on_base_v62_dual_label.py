...
BUY_FILTERS = [
    BuyFilter(rs, amt, wpos, depth, bpct, ret80, b2min, b2max, vcon, legmax)
    for rs in [0.50, 0.60]
    for amt in [1.00]
    for wpos in [0.80]
    for depth in [0.15, 0.20]
    for bpct in [0.12]
    for ret80 in [0.80, 1.20]
    for b2min, b2max in [(5, 45), (5, 60), (8, 60)]
    for vcon in [0.90, 1.10, 1.50]
    for legmax in [0.60, 1.00]
]

EXIT_RULES = [
    ExitRule(hard_stop=hs, fail_window_days=fw, fail_pivot_buffer=0.97, ma20_after_days=ma, ma20_confirm_days=confirm, trailing_stop=tr, max_hold_days=60)
    for hs in [0.08]
    for fw in [3]
    for ma in [15, 20]
    for confirm in [2]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="base_on_base V6.2 strict + loose 双标签实验")
...
def summarize(validation: pd.DataFrame, buy_filter: BuyFilter, exit_rule: ExitRule, min_samples: int) -> dict:
    valid = validation[validation["valid_entry"] == True].copy()
    valid = valid[
        (
            (valid["v62_label"] == "strict")
            | (
                (valid["v62_label"] == "loose")
                & (valid["entry_gap_from_signal_close"].fillna(99) <= 0.08)
                & (valid["entry_over_pivot_hfq"].fillna(99) <= 0.15)
            )
        )
    ].copy()
...
def by_year(validation: pd.DataFrame, label: str) -> pd.DataFrame:
    valid = validation[validation["valid_entry"] == True].copy()
    valid = valid[
        (
            (valid["v62_label"] == "strict")
...
def main() -> None:
    ...
    for buy_filter in BUY_FILTERS:
        ...
        for exit_rule in EXIT_RULES:
            ...
            summary_rows.append(summarize(validation, buy_filter, exit_rule, args.min_samples))
    ...
    summary_path = lab_dir / f"base_on_base_v62_strategy_grid_{stamp}.csv"
    summary.to_csv(summary_path, index=False)

    best = summary.head(20).copy()
    best_path = lab_dir / f"base_on_base_v62_top20_{stamp}.csv"
    ...
    for _, row in best.head(5).iterrows():
        ...
        validation = validation[
            (
                (validation["v62_label"] == "strict")
                | (
                    (validation["v62_label"] == "loose")
                    & (validation["entry_gap_from_signal_close"].fillna(99) <= 0.08)
                    & (validation["entry_over_pivot_hfq"].fillna(99) <= 0.15)
                )
            )
        ].copy()
        label = f"{row['buy_filter']}__{row['exit_rule']}"
        year_parts.append(by_year(validation, label))
        valid = validation[validation["valid_entry"] == True].copy()
        ...
        detail_path = lab_dir / f"validation_v62_{label[:116]}_{stamp}.csv"
        validation.to_csv(detail_path, index=False)
        detail_paths.append(detail_path)

    year_path = lab_dir / f"base_on_base_v62_top5_by_year_{stamp}.csv"
    reasons_path = lab_dir / f"base_on_base_v62_top5_exit_reasons_{stamp}.csv"
    ...
    logger.info("网格汇总：%s", summary_path)
    logger.info("Top20：%s", best_path)
    logger.info("Top5 分年：%s", year_path)
    logger.info("Top5 卖出原因：%s", reasons_path)
    logger.info("Top5 明细：%s", [str(p) for p in detail_paths])
