...
class BuyFilter:
    ...
    def name(self) -> str:
        return (
            ...
            f"bpct{self.breakout_pct_max:.2f}_ret80{self.ret80_max:.1f}"
        ).replace(".", "p")
...
BUY_FILTERS = [
    BuyFilter(rs, amt, wpos, depth, bpct, ret80)
    for rs in [0.50, 0.60]
    for amt in [1.00, 1.20]
    for wpos in [0.80]
    for depth in [0.15, 0.20]
    for bpct in [0.12, 0.15]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="base_on_base V5 真实交割口径 + 平台/过热/卖点优化实验")
